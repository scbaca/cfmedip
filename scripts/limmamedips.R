MEDIPS.meth = function(
		...
		minRowSum=10,
		diffnorm="tmm",
		depth = NULL #a dataframe containing depth info to be included as a covariate in limma model if specified
		)
{
	...
	if(!is.null(MSet1) & !is.null(MSet2)){
		...
		if(diff.method=="edgeR"|diff.method=="limma"){
		    ...
		    if(diff.method=="edgeR"){
			  ...
			  diff.results.list = MEDIPS.diffMeth(base=base, values=counts.medip, diff.method="limma", nMSets1=nMSets1, nMSets2=nMSets2, p.adj=p.adj, n.r.M1=n.r.M1, n.r.M2=n.r.M2, MeDIP=MeDIP, minRowSum=minRowSum, diffnorm=diffnorm, depth=depth)
			}
...
MEDIPS.diffMeth = function(base = NULL, values=NULL, diff.method="ttest", nMSets1=NULL, nMSets2=NULL, n.r.M1=n.r.M1, n.r.M2=n.r.M2, p.adj="bonferroni", MeDIP, minRowSum=10, diffnorm="tmm", depth=NULL)
{
	##edgeR##
	#########
	if(diff.method=="edgeR"){		
		...
		if(nMSets1!=1 | nMSets2!=1){
			...
			if(!is.null(depth)) {
				design <- model.matrix(~edRObj.group + depth$depth)
			} else {
            ...
            v <- limma::voom(d, design) #use voom to model variance as a function of counts 
